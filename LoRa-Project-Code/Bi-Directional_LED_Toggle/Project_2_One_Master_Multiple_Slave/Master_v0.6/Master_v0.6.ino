...
void loop(){
  for(nodeNumber=1;nodeNumber<(NUMBER_OF_NODES+1);nodeNumber++){//Useful for multiple nodes, for 1 node runs only once
    ...
    if (Serial.available()) {  
      ...
      switch(readVal){
        case 0:
          strToSend=stringToHexString(MASTER_CODE_OFF)+"3A"+stringToHexString(String("1"));
          digitalWrite(LED_PIN, LOW);
          //Serial.println('0');
          break;
        case 1:
          strToSend=stringToHexString(MASTER_CODE_ON)+"3A"+stringToHexString(String("1"));
          digitalWrite(LED_PIN, HIGH);
          //Serial.println('1');
          break;
        case 2:
          strToSend=stringToHexString(MASTER_CODE_OFF)+"3A"+stringToHexString(String("2"));
          digitalWrite(LED_PIN, LOW);
          //Serial.println('0');
          break;
        case 3:
          strToSend=stringToHexString(MASTER_CODE_ON)+"3A"+stringToHexString(String("2"));
          digitalWrite(LED_PIN, HIGH);
          //Serial.println('1');
          break;
        default:
    ...
    if ( str.indexOf("ok") == 0 ){
      ...
      if ( str.indexOf("radio_rx") == 0 ){
        ...
        //Serial.println(decodeReceivedString(strRecd,"3A",":"));//Getting DataString from HEXDataString eg. 343:22:56:
      }
      else{
