/* This is for the Master Device
 *  MASTER transmits (Tx) the MASTER_CODE + nodeNumber
 *  and waits to receive (Rx) a string of sensor values from the that nodeNumber (i.e. Slave 1)
 *  Then sends this string of values over Serial Monitor which is captured by the Raspberry Pi
 *  
*/
...
void loop(){
  for(nodeNumber=1;nodeNumber<(NUMBER_OF_NODES+1);nodeNumber++){//Useful for multiple nodes, for 1 node runs only once
    ...
    Serial.println("Sending to RasPi "+hexStringToString(strToSend)+":"+decodeReceivedString(strRecd,"3A",":"));
